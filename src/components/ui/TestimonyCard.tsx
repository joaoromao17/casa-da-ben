...
const TestimonyCard = ({ name, date, content, isAnonymous = false }: TestimonyCardProps) => {
  const formattedDate = format(date, "dd 'de' MMMM 'de' yyyy", { locale: ptBR });
  const displayName = isAnonymous ? "Anônimo" : name;

  const shareOnWhatsApp = () => {
    const text = `Testemunho de ${displayName}: "${content}" - Compartilhado da Igreja Casa da Benção`;
